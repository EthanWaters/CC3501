...
def main(sock):
    ...
    while True:
        ...
        if (gps_recieved_counter <= gps_required_for_start) & gps_visual_update_flag:
            ...
            # Ready to send:
            result_line = f"{lat_delta_meters:> 3.1f}, {lon_delta_meters:> 3.1f}, {predictedLat:>.6f}, {predictedLon:>.6f}, {yaw_mapped:> 6.1f}, {obj_north.getPredictedVel():> 4.1f}, {obj_east.getPredictedVel():> 4.1f}"
            result_dict = {'lat_meters': lat_delta_meters, 'lon_meters': lon_delta_meters, 'yaw': yaw_mapped}
            ...
            if (time.time() - serial_loop_timer) >= (target_delay):
                # TODO: send using sockets:
                sock.send(str(result_dict).encode())
                serial_loop_timer = time.time()    
...
if __name__ == "__main__":
    ...
    with socket.socket() as sock:
        try:
            # sock.connect(('192.168.0.5', 12345))
